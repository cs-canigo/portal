...
## Suport per a Arxius de Gran Tamany

Podem incloure arxius en el nostre repositori que superen la limitació de 25 MB utilitzant l'extensió [git-lfs](https://git-lfs.github.com/).

Aquest mòdul ens permet integrar fitxers d'audio, video... en el nostre codi font sense incrementar el tamany en el repositori central, evitant posibles problemes de rendiment en el servidor.

Per a utilitzar-lo dintre d'un projecte cal que **tots** els col·laboradors instalin l'extensió en el lloc de treball. Les instruccions d'instalació les podeu trobar a:

* [Windows](https://github.com/git-lfs/git-lfs/wiki/Installation#windows)
* [Linux](https://github.com/git-lfs/git-lfs/wiki/Installation#debian-and-ubuntu) 

Un cop instalat, en qualsevol projecte podem indicar les extensions dels fitxers que cal incloure en el git-lfs amb la següent comanda en l'arrel del projecte:

	git lfs track "*.iso" 


Això generarà un fitxer **.gitattributes** que haurem d'incloure en el nostre projecte git.
A partir d'aquest moment automaticament tots els fitxer amb extensió .iso es pujaran en el mòdul lfs del servidor i aixi podrem evitar les restriccions de tamany.

**lfs** accepta expresions regulars semblants a les que podem trobar en el fitxer .gitignore, per tant el tracking pot ser tant d'un fitxer com de grups de fitxers per extensió o altres configuracions que siguin necessaries.
...
## Notificacions via Correu o Slack

El gitlab permet que configurem notificacions per a rebre via email activitats que s'han produit en els nostres projectes.

Cada usuari pot configurar les notificacions que vol rebre accedint a l'apartat User -> Settings -> Notifications dintre del Gitlab.

![Notificacions Usuari](/related/sic/userNotifications.png)  
<br/>
La documentació oficial la podem trobar [aquí](https://docs.gitlab.com/ce/workflow/notifications.html)
...
[Més informació](https://docs.gitlab.com/ee/user/project/integrations/emails_on_push.html)
